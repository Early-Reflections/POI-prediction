...
from utils import seed_torch, set_logger, Cfg, count_parameters, test_step, save_model,visualize_channel_weight
...
if __name__ == '__main__':
    ...
    if cfg.run_args.do_train:
        ...
        sampler_train = accelerator.prepare(sampler_train)
    ...
    if cfg.run_args.do_validate:
        ...
        sampler_validate = accelerator.prepare(sampler_validate)
    ...
    if cfg.run_args.do_test:
        ...
        sampler_test = accelerator.prepare(sampler_test)
    ...
    if cfg.run_args.do_train:
        ...
        best_metrics = 0.0
        global_step = 0
        if cfg.run_args.visualize:
            # attention_weights_00 = []
            # attention_weights_01 = []
            # attention_weights_10 = []
            # attention_weights_11 = []
            # attention_weights_20 = []
            # attention_weights_21 = []
            attention_weights_0 = []
            attention_weights_1 = []
            attention_weights_2 = []
        for eph in range(cfg.run_args.epoch):
            ...
            for data in tqdm(sampler_train):
                model.train()
                split_index = torch.max(data.adjs_t[1].storage.row()).tolist()  #checkin idx max
                data = data.to(device)
                input_data = {
                    'x': data.x,
                ...
                if cfg.run_args.do_validate and global_step % cfg.run_args.valid_steps == 0:
                    logging.info(f'[Evaluating] Evaluating on Test Dataset...')
                    recall_res, ndcg_res, map_res, mrr_res, eval_loss = test_step(model, data=sampler_test)
                    summary_writer.add_scalar(f'test/Recall@1', 100*recall_res[1], global_step)
                    summary_writer.add_scalar(f'test/Recall@5', 100*recall_res[5], global_step)
                    summary_writer.add_scalar(f'test/Recall@10', 100*recall_res[10], global_step)
                    summary_writer.add_scalar(f'test/Recall@20', 100*recall_res[20], global_step)
                    summary_writer.add_scalar(f'test/MRR', mrr_res, global_step)
                    summary_writer.add_scalar(f'test/eval_loss', eval_loss, global_step)
                    logging.info(f'[Evaluating] Recall@1: {100*recall_res[1]}, Recall@5: {100*recall_res[5]}, Recall@10: {100*recall_res[10]}, Recall@20: {100*recall_res[20]}, MRR: {mrr_res}, eval_loss: {eval_loss}')
                ...
                global_step += 1
            if cfg.run_args.visualize:
                # attention_weights_00.append(model.conv_for_time_filter.feature_attention_source.attention_weights.cpu().detach())
                # attention_weights_01.append(model.conv_for_time_filter.feature_attention_target.attention_weights.cpu().detach())
                # attention_weights_10.append(model.conv_list[0].feature_attention_source.attention_weights.cpu().detach())
                # attention_weights_11.append(model.conv_list[0].feature_attention_target.attention_weights.cpu().detach())
                # attention_weights_20.append(model.conv_list[1].feature_attention_source.attention_weights.cpu().detach())
                # attention_weights_21.append(model.conv_list[1].feature_attention_target.attention_weights.cpu().detach())
